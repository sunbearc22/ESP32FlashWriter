...
class ESP32Device(ttk.Labelframe):
    ...
    def _port_is_busy( self, port):
        print('\ndef _port_is_busy( self, portName):')
    ...
    def _create_esp_connection( self ):
        ...
        try:
            print('self.esp =', self.esp)
            if self.esp:
                print('self.esp._port.isOpen() =', self.esp._port.isOpen())
            ...
            if self.esp:
                ...
                try: 
                    ...
                    if 'Linux' in platform.system():
                        ...
                        self._check_connection_Win() # Check esp32 device is connected every 1s.                        
                #print( status )
                self.connected = True
            else:
    ...
    def _check_connection_Win( self ):
        '''Check ESP32 connection is ok or broken for Windows. 
        ...
        if self._connected:
            ...
            print('self._connected = False')
            self.status.set( ESP32Device.MSG0 )
            self._sop_for_not_connected()


    def _check_connection( self ):
        '''Check ESP32 connection is ok or broken for Linux. 
...
class FlashFirmware(ttk.Labelframe):
    ...
    def _set_erase_all( self, event ):
        print( 'def _set_erase_all( self, event ):' )
        print( 'self._erase_all.get() = ', self._erase_all.get() )
